...
class MirrorListener(listeners.MirrorListeners):
    ...
    def onDownloadError(self, error):
        ...
        msg = f"{uname} your download stopped..☠️: {error}"
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ):
        if self.isLeech:
            ...
            if self.message.chat.type == 'private':
                msg = f'<b>⌈➳ ɳαɱε 💌 :</b> <code>{link}</code>\n'
                msg += f'<b>⌈➳ 🌩 𝚃𝙾𝚃𝙰𝙻 𝙵𝙸𝙻𝙴𝚂 : </b> {count} ⌖'
                sendMessage(msg, self.bot, self.update)
            else:
                chat_id = str(self.message.chat.id)[4:]
                msg = f"<b>⌈➳ 💌 𝙵𝙸𝙻𝙴𝙽𝙰𝙼𝙴 ♻ : </b> <a href='https://t.me/c/{chat_id}/{self.uid}'>{link}</a>\n"
                msg += f'<b>⌈➳ 🗂 𝚃𝙾𝚃𝙰𝙻 𝙵𝙸𝙻𝙴𝚂 => : </b> {count}\n'
                msg += f'⌈➳ Leech ᏰᎩ ⇆ 👰 {uname}\n\n⌈➳ INFO ⇆ :'
                fmsg = ''
            ...
            return
        with download_dict_lock:
            msg = f'<b>⌈➳ 💌 ғɪʟᴇ-ɴᴀᴍᴇ : </b><code>{download_dict[self.uid].name()}</code>\n\n<b>⌈➳ 💎 ᎪᏴϴႮͲ ՏᏆᏃᎬ  : </b><code>{size}</code>\n\n<b>=> ᴅᴏɴ༝ᴛ sʜᴀʀᴇ ᴛʜᴇ ɪɴᴅᴇx ʟɪɴᴋ ᴏᴜᴛsɪᴅᴇ ᴛʜɪs ɢʀᴏᴜᴘ..! 😎\n</b>\n<b>⌈➳ 🧰 мαℓωαяєѕ : ղօ ✅</b>'
            if os.path.isdir(f'{DOWNLOAD_DIR}/{self.uid}/{download_dict[self.uid].name()}'):
                msg += '\n<b>⌈➳ ♻ 𝚃𝚈𝙿𝙴 : </b><code>⌈➳ 🗂 ҒϴᏞᎠᎬᎡ </code>'
                msg += f'\n<b>⌈➳ 📁 ՏႮᏴ-ҒϴᏞᎠᎬᎡ : </b><code>{folders}</code>'
                msg += f'\n<b>⌈➳ 📝 ғɪʟᴇs : </b><code>{files}</code>'
            else:
                msg += f'\n<b>♻ 𝚃𝚈𝙿𝙴 : </b><code>{typ} ⌘</code>'
            buttons = button_build.ButtonMaker()
            if SHORTENER is not None and SHORTENER_API is not None:
                surl = short_url(link)
                buttons.buildbutton("🌩 𝙳𝚁𝙸𝚅𝙴-𝙻𝙸𝙽𝙺 🌩", surl)
            else:
            ...
            if INDEX_URL is not None:
                ...
                if os.path.isdir(f'{DOWNLOAD_DIR}/{self.uid}/{download_dict[self.uid].name()}'):
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        ...
                        buttons.buildbutton("⚡ Ꮖᑎᗞᗴ᙭-𝐋𝐈𝐍𝐊 🔰", siurl)
                    else:
                        buttons.buildbutton("⚡ Ꮖᑎᗞᗴ᙭-𝐋𝐈𝐍𝐊 🔰", share_url)
                else:
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        ...
                        if VIEW_LINK:
                            ...
                            buttons.buildbutton("✅ ᐯᏆᗴᗯ-しᏆᑎᏦ 💝", siurls)
                    else:
                        buttons.buildbutton(" 🔰 Ꮖᑎᗞᗴ᙭-𝐋𝐈𝐍𝐊 🔰", share_url)
                        if VIEW_LINK:
                            buttons.buildbutton("✅ ᐯᏆᗴᗯ-しᏆᑎᏦ 💝", share_urls)
...
def _mirror(bot, update, isTar=False, extract=False, isZip=False, isQbit=False, isLeech=False):
    ...
    if bot_utils.is_url(link) and not bot_utils.is_magnet(link) and not os.path.exists(link) and isQbit:
        ...
        sendMessage('No download source provided..☠️', bot, update)
        return
    ...
    if bot_utils.is_gdrive_link(link):
        ...
        if BLOCK_MEGA_LINKS:
            sendMessage("Mega links are blocked.. Don't Try again..☠️!", bot, update)
            return
        link_type = bot_utils.get_mega_link_type(link)
        if link_type == "folder" and BLOCK_MEGA_FOLDER:
            sendMessage("Mega folder are blocked..Don't Try again..☠️!", bot, update)
        else:
